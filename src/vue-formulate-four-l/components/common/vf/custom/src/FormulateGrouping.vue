...
export default {
  ...
  computed: {
    items() {
      if (Array.isArray(this.context.model)) {
        ...
        if (this.context.model.length < this.context.minimum) {
          return new Array(this.context.minimum || 1)
            .fill("")
            .map((t, index) =>
              this.setId(this.context.model[index] || {}, index)
            );
        }
      ...
      return new Array(this.context.minimum || 1)
        .fill("")
        .map((_i, index) => this.setId({}, index));
      */
      //var a = new Array(this.context.minimum || 1)
      //  .fill("")
      //  .map((_i, index) => this.setId({}, index));
    ...
    groupErrors() {
      return this.items.map((item, index) =>
        has(this.context.groupErrors, index)
          ? this.context.groupErrors[index]
          : {}
      );
    },
  },
  ...
  methods: {
    validateGroup() {
      return Promise.all(
        this.providers.reduce((resolvers, provider) => {
        ...
        (providersHasErrors) =>
          !providersHasErrors.some((hasErrors) => !!hasErrors)
      );
    },
    showErrors() {
      this.providers.forEach(
        (p) => p && typeof p.showErrors === "function" && p.showErrors()
      );
    },
    setItem(index, groupProxy) {
      ...
      if (
        ...
        !this.context.model.__init
      ) {
        this.context.model.splice(index, 1, this.setId(groupProxy, index));
      } else {
        this.context.model = this.items.map((item, i) =>
          i === index ? this.setId(groupProxy, index) : item
        );
      }
    },
    removeItem(index) {
      if (
        ...
        this.context.model = this.context.model.filter((item, i) =>
          i === index ? false : item
        );
        this.context.rootEmit("repeatableRemoved", this.context.model);
      } else if (
        !Array.isArray(this.context.model) &&
        this.items.length > this.context.minimum
      ) {
        // In this context the fields have never been touched (not "dirty")
        this.context.model = new Array(this.items.length - 1)
          .fill("")
          .map((_i, idx) => this.setId({}, idx));
        this.context.rootEmit("repeatableRemoved", this.context.model);
      }
  ...
  mounted() {
    //console.log(`were in!`, this.items);
    if (
      this.items &&
      Object.keys(this.items)?.length == 1 &&
      this.items[0].hasOwnProperty("__id")
    ) {
      console.log(`do something!`, this.context.name);
      this.removeItem(0);
    }
  },
};
