...
*/
    ...
    const trySubmit = async () => {
      ...
      // console.log("formRef1", formRef);
      //console.log("context", context);
      // this.$formulate.submit("login"); // cant use this!

      // expect(wrapper.findAll('.formulate-input-errors li').length).toBe(1);

      //console.log("has errors", errors.length);

      // the source code examples use await
      await formRef.value.formSubmitted();

      const errors = document.querySelectorAll(
        "form[name='myForm'] > .formulate-input > .formulate-input-errors > li"
      );

      errors[0].scrollIntoView();

      /*
      const fs = formRef.value.formSubmitted().then((a) => {
        const errorsInside = document.querySelectorAll(
          "form[name='myForm'] > .formulate-input > .formulate-input-errors > li"
        );
        console.log("inside fs", fs);
        console.log("inside errors", errorsInside.length);
      });
      */

      //console.log("outside fs", fs);
      console.log("outside errors", errors.length);
    };
...
<template>
  <div class="p-5">
    <FormulateForm
      ...
      ref="formRef"
    >
      <input type="button" value="Submit form" @click="trySubmit" />
    </FormulateForm>
    <pre>{{ data.model }}</pre>
  </div>
</template>
